...
if ( ! class_exists('santoi_Lib_Library')) {

	class santoi_Lib_Library {
		...
		private function __construct() {
			...
			self::$plugin_data = array(
			 'root_file' =>  __FILE__,
			 'pro-link' => '#',
			 'remote_site' => 'https://api.themepreview.xyz/santoi/',
			 'all_endpoint' => 'santoi_lib_all',
			 'single_endpoint' => 'santoi_single_lib'
			);
			...
			add_action( 'wp_ajax_santoi_lib_reload_template', array($this,'reload_library'));
		...
		public function editor_script(){
			...
			wp_enqueue_script( 'santoi-lib-library',  plugins_url( '/assets/js/elementor-manage-library.js', __FILE__));
		...
		function ajax_data(){
			...
			$nav = '';
			$products = get_option('santoi_lib_library');
			...
			if ( is_array($products) ){
				...
				if (count($paged_products)) {
					foreach ($paged_products as $product) {
						...
						if( $product['pro'] && !class_exists('santoi_Lib_Pro') ){
